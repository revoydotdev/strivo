...
| Windows build | 🟡 | Toolchain blocker cleared 2026-08-18: `win11-ci` now has VS Build Tools (cl/link 14.44.35207 + Win11 SDK), Rust 1.97.1 MSVC, ffmpeg 9.0.1 at `C:\ffmpeg-win64`, and git. A real MSVC `cargo check` there now fails **only** on the port itself — 6 errors: `tokio::net::Unix{Listener,Stream}` in `src/daemon.rs` and `src/playback/mod.rs`, `std::os::unix::net` and two `libc::kill` calls in `src/ipc.rs`. `scripts/check-windows.sh` reproduces those same 6 from Linux via the GNU target, so the port can be developed without the VM. Remaining: the transport swap, and graceful child termination — `Recorder::stop()`'s `#[cfg(not(unix))]` arm hard-kills ffmpeg, so no Matroska trailer is written |

...
- **`M2.P1.S2.T3`** — Migrate the 5 true cross-recording *analytics* handlers in `crates/strivo-web/src/routes/plugins.rs` (`insights_words`, `insights_topics`, `insights_speakers`, `insights_export`, `insights_compare` — the ones that map 1:1 onto `T2`'s 4 signal kinds) off the raw `crunchr.db` `rusqlite::Connection` to `SignalStore::open(data_dir/signals.db)` + the query API, in lockstep with `insights` `T1`. → *Artifact:* `cargo test --workspace --features creator insights_via_signal_store` · *Concern:* webui-signals
  - **Rescoped tick 2026-07-19f** (owner decision, supersedes the "Unshippable as literally scoped" note from 2026-07-19e): the prior whole-file-grep artifact conflated crunchr's own *operational* crunchr.db reads (captions/chapters/transcript/CRUD — ~22 sites, legitimate: crunchr owns crunchr.db) with the 5 true cross-recording analytics reads this concern actually targets. VISION AX-6's canonical store is for *signals* (word_freq/airtime/sentiment/topic), not raw transcript text, so mirroring full per-segment transcript (including unlabeled-speaker segments) into the signal store would be a category error and a data-duplication trap — see superseded `M2.P1.S2.T4` below. New artifact drops the file-wide grep and asserts behavior instead: `insights_via_signal_store` seeds `signals.db` and checks the 5 handlers return the seeded data — a handler still reading crunchr.db would return different/empty output, so the test proves the migration without over-constraining files it doesn't own. `insights_retention` and all other crunchr.db web reads are explicitly OUT of scope and stay on crunchr.db.
- ~~`M2.P1.S2.T4`~~ — **SUPERSEDED (tick 2026-07-19f).** Was: extend the `crunchr` signal-store producer to also mirror unlabeled-speaker segments so the full-file grep could close. Superseded because `T3` was rescoped (above) to its true 5-handler scope instead — mirroring raw transcript text into the signal store is a wrong abstraction (VISION AX-6 is for derived signals, not transcript duplication) and the concern it existed to unblock no longer exists. No further action.
...
- **M2G2** — no plugin reaches into a sibling plugin's SQLite (crunchr's own reads of its own `crunchr.db` are excluded — crunchr owns crunchr.db). → *Check:* `bash -c '! git grep -qn "crunchr.db" -- crates/strivo-plugins/src ":!crates/strivo-plugins/src/crunchr"'`

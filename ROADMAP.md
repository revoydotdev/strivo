...
- **`M2.P1.S2.T3`** — Migrate the webui analytics call sites (`crates/strivo-web/src/routes/plugins.rs` + `crates/strivo-web/tests/plugins_data.rs`, ~9 sites) off the raw `crunchr.db` `rusqlite::Connection` to `SignalStore::open(data_dir/signals.db)` + the query API, in lockstep with `insights` `T1`. → *Artifact:* `bash -c 'cargo test --workspace --features creator insights_via_signal_store && ! git grep -qn "crunchr.db" -- crates/strivo-web/src'` · *Concern:* webui-signals
  - **Unshippable as literally scoped** (found tick 2026-07-19e): `plugins.rs` has ~27 `crunchr_db()`/`crunchr.db` sites, not ~9. Only 5 are true cross-recording *analytics* mappable 1:1 onto the 4 kinds `crunchr-signals` (`T2`) mirrors — `insights_words`, `insights_topics`, `insights_speakers`, `insights_export`, `insights_compare`. Built + verified in isolation this tick (`insights_via_signal_store` passes; a `signals_db()`-based `SignalStore` migration replaces the 5 handlers' `open_ro(&crunchr_db())`), but NOT committed — the todo's own artifact still fails because ~20 other sites remain: `chapters_generate` (×2), `captions_export`, `insights_retention`, `recording_captions`, plus brandsafe/reuse/casebook/heatmap/broll/crunchr-CRUD. These need full per-segment transcript text — including segments with **no speaker label**, which the `T2` producer's `speaker_segment` kind deliberately skips (see `crunchr/db.rs::load_full_segments_preserves_speakers_and_order` — real transcripts do have unlabeled segments, e.g. when diarization is off) — so migrating them would silently drop spoken content from live captions/VTT. `chapters_generate` additionally calls `strivo_chapters::generate_chapters(&Path, ..)`, a different crate outside this concern's 2-file scope, and (bonus finding) is *already dead*: its `read_segments` queries a `segments.recording_id` column that doesn't exist in the real `crunchr.db` schema (only `video_id`), so it errors on any real recording today — not a regression risk either way. Two ways to actually close `T3`: (a) `M2.P1.S2.T4` below, or (b) rescope this todo's artifact to the 5 analytics handlers only (mirrors the still-open M2G2 gate-wording gap noted at tick `2026-07-19c`). Left the artifact untouched pending that decision — did not force a pass. Worktree/branch destroyed; the 5-handler migration + test are not preserved anywhere, re-do them once `T4` lands or the artifact is rescoped.
- **`M2.P1.S2.T4`** — Extend the `crunchr` signal-store producer (`T2`) to also mirror segments with no speaker label (e.g. a new `transcript_segment` kind covering every segment regardless of diarization), so `captions_export`/`insights_retention`/`recording_captions` can migrate off `crunchr.db` without losing spoken content; then re-attempt `T3`'s full-file grep. Alternatively, an owner may instead rescope `T3`'s artifact to the 5 analytics handlers and close it directly without this todo. → *Artifact:* TBD by owner decision · *Concern:* signal-store-full-parity
